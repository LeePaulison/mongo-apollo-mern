...
recordRoutes.route("/record/:id").get(async function (req, res) {
  ...
  await db_connect.collection("records").findOne(myquery, function (err, result) {
    if (err) throw err;
    res.json(result);
  });
});
...
recordRoutes.route("/update/:id").post(async function (req, response) {
  ...
  await db_connect.collection("records").updateOne(myquery, newvalues, function (err, res) {
    if (err) throw err;
...
recordRoutes.route("/:id").delete(async function (req, response) {
  let db_connect = dbo.getDb();
  let myquery = { _id: new ObjectId(req.params.id) };
  await db_connect.collection("records").deleteOne(myquery, function (err, obj) {
    if (err) throw err;
